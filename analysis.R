...
texpage <- function(){
"
Rule {{nr}}: Natural language.
\\begin{quote}
{{NL}}
\\end{quote}
The VTL implementation.
\\begin{verbatim}
{{{VTL}}}
\\end{verbatim}
The eStatistik implementation.
\\begin{verbatim}
{{eStatistik}}
\\end{verbatim}
...
I <- grep("```",lines)
lines[I[c(TRUE,FALSE)]] <- "\\begin{verbatim}"
lines[I[c(FALSE,TRUE)]] <- "\\end{verbatim}"
txt <- paste0(lines,collapse="\n")
txt <- strsplit(txt,"#####",fixed=TRUE)[[1]][-1]
rules <- str_extract(txt,".*?[0-9]{1,2}") 
txt %<>% str_replace(".*?[0-9]{1,2}","") %>%     # remove rule number prefix
